...
export default class Level {
  ...
  movingCrateIndex: number;
  crateIsMoving: boolean;

  isComplete: boolean;

  constructor(levelMap: TileType[]) {
    this.reset(levelMap);
  }

  reset(levelMap: TileType[]): void {
    console.log('reset');
    ...
    for (let r = 0; r < GRID_ROWS; r++) {
      for (let c = 0; c < GRID_COLS; c++) {
        ...
        switch (type) {
          ...
          case TileType.Node: {
            this.nodes.push(new TeslaNode(posX, posY, tile as GroundTile));
            break;
          }
        }
  ...
  updateCoilPaths(): void {
    if (this.coils.length < 2) return;

    this.tiles.filter(tile => tile instanceof GroundTile).forEach(tile => { (tile as GroundTile).isPath = false; });
  ...
  updateSelectedCrateIndex(index: number): void {
  ...
  findPath(startTile: Tile, endTile: Tile, usePaths: boolean): PathNode {
    let firstNode: PathNode = null;
    let lastNode: PathNode = null;

    const closedSet: PathNode[] = [];
    const openSet: PathNode[] = [];
    const finalPath: PathNode[] = [];
  ...
  getHeuristic(a: Tile, b: Tile): number {
    return round((abs(a.pos.x - b.pos.x) + abs(a.pos.y - b.pos.y)) / TILE_SIZE);
  }

  isBlocked(tile: Tile, pathsCanBlock: boolean): boolean {
    return (tile instanceof WallTile) ||
      (pathsCanBlock && (tile as GroundTile).isPath) ||
